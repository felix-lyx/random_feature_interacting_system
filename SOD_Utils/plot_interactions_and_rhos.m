...
ax                             = gca();
set(gca, 'YColor', [0.4549 0.1137 0.8471]);
...
if ~isempty(phir)
  ...
  phiPlotHandles(PPH_count)    = plot(r, phir, 'k', 'LineWidth', plot_info.phi_line_width+1); hold on;% plot the true phi, put on a hold
...
if plot_info.display_phihat || ~plot_info.display_interpolant
  PPH_count                    = PPH_count + 1;
  phiPlotHandles(PPH_count)    = plot(r, mean(phihatr,1), '-r', 'LineWidth', plot_info.phihat_line_width+2.5);% plot the learned phi
...
if plot_info.display_interpolant
  ...
  phiPlotHandles(PPH_count)    = plot(r, mean(phihatsmoothr,1),  '--', 'Color', [0. 0., 255.]/255, 'LineWidth', plot_info.phihat_line_width+3);
...
set(legendHandle, 'Location', 'best', 'Interpreter', 'latex', 'FontSize', plot_info.legend_font_size, 'FontName', plot_info.legend_font_name);
y_range                        = y_max - y_min;
